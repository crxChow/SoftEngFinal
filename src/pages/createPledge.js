...
export default function CreatePledge({ pledgeModel, pModel }) {
  ...
  const projID = window.location.href.split('/projects/')[1].split('/create')[0];
  console.log(projID);

  function handleMove() {
    let createModel = new Model();
    createModel.addDesigner(pledgeModel.designer.email, pledgeModel.designer.did);
    ...
    for(let i=0;i<pledgeModel.designer.projects.length; i++){
        ...
        if(proj.pid == projID){
            console.log(proj);
            /*createModel.designer.addProject(
                proj.name,
                proj.pid,
                proj.description,
                proj.date,
            ...
            thisProj = proj;
            pledgeModel.designer.projects.splice(pledgeModel.designer.projects.indexOf(proj), 1);
        }
    }
    ...
    instance.post("/createpledge", js).then((response) => {
      ...
      if (response.data.status === 200) {
        let project = response.date.result;
        console.log(response.date.result);
        //thisProj.pledge = response.date.result;
        console.log(project);
